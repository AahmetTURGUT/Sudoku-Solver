...
import static sudoku.ThreadB.CLOSED_CELL_TEXT;
import static sudoku.ThreadA.cozum;
import static sudoku.ThreadB.GrafikAdiB;
import static sudoku.ThreadA.GrafikAdiA;
import static sudoku.ThreadA.ekranaYazdir;
import static sudoku.ThreadB.cozum;

public class ThreadC implements Runnable {

    static List<String> bb = new ArrayList<String>();
    private String ad;
    static String GrafikAdiC = " ";
    public static Color CLOSED_CELL_TEXT;

    public ThreadC(String ad) {
        this.ad = ad;
    }
    ...
    public void run() {
       ...
       ii = generator.nextInt(800);
        ...
        if (cozum(i, j, tahta, 0, randDeger) && Main.bittiMi != true) {
            ...
            GrafikAdiC = "Board C=" + (end - start) + " MS DE BULDU";
            ekranaYazdir(tahta);
        ...
        for (i = 0; i < 9; i++) {
            ...
            for (j = 0; j < 9; j++) {
                ...
                if (TahtadanYazdir[i][j] == 0) {
                    ...
                    GrafikC.puzzle[i][j] = TahtadanYazdir[i][j];
                }
            }
            System.out.println("|");
        }
